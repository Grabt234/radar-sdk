...
class Statistic:
    ...
    @classmethod   
    def std(cls, df: pl.DataFrame, header: str, filters: list[FilterTuple] | None = None, combine_method: Literal["AND", "OR"] = "AND") -> float:
        if filters:
    ...
    @classmethod
    def mean(cls, df: pl.DataFrame, header: str, filters: list[FilterTuple] | None = None, combine_method: Literal["AND", "OR"] = "AND") -> float:
        if filters:
            df = cls._filter_by_bounds(df, filters, combine_method)
        val = df[header].mean()
        val = cast(float | None, val)
    ...
    @classmethod
    def max(cls, df: pl.DataFrame, header: str, filters: list[FilterTuple] | None = None, combine_method: Literal["AND", "OR"] = "AND") -> float:
        if filters:
            df = cls._filter_by_bounds(df, filters, combine_method)
        val = df[header].max()
        val = cast(float | None, val)
