...
class Community:
    ...
    def __init__(
        ...
        if not (0.0 <= kill_prop < 1.0):
            raise ValueError(
                "kill_prop must be between 0.0 (inclusive) and 1.0 (exclusive)."
            )
    ...
    def sort(self, fitness_fn: Callable[[Organism], dict]) -> None:
        ...
        self._fittest_organism = (
            self._fitness_map[self._organisms[0]],
            self._organisms[0],
        )
    ...
    def propagate(self, fitness_fn: Callable[[Organism], dict]) -> None:
        ...
        if (
            self._fittest_organism[0] is None
            or len(self._organisms) > self.community_size
        ):
