...
class Population:
    ...
    def __init__(
        ...
        self._init_communitites(num_communities, community_sizes,headers, num_values_per_chrom,min_vals, max_vals, kill_prop)
    ...
    def _init_communitites(self,num_communities : int, community_sizes : int, headers : List[str], num_values_per_chrom, min_vals : List[float], max_vals : List[float], kill_prop):
    ...
    def propogate(self, fitness_fn: Callable[[Organism], float], elite = False, plot = True, generation=0):
        ...
        if not elite:
            ...
            self._elite_community._organisms = self._elite_community._organisms[:self._community_sizes]
